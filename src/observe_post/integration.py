...
from .observeinc import ObserveInc
...
class Integration:
    ...
    def __init__(self, config, event_type_fields_mapping, initial_delay):
        ...
        for instance in config['instances']:
            ...
            labels['nr-labs'] = 'data'
            # client = AWS_S3(instance_name, instance['arguments'], event_type_fields_mapping, initial_delay)
            oauth_type = instance['arguments']['auth']['grant_type']
            self.instances.append({'labels': labels, 'client': client, "oauth_type": oauth_type})
        observeinc_config = config['observe']
        data_format = observeinc_config['data_format']
        if data_format.lower() == "logs":
            self.data_format = DataFormat.LOGS
            ObserveInc.logs_license_key = observeinc_config['customer_id']
            ObserveInc.set_logs_endpoint(observeinc_config['ingest_token'])
        elif data_format.lower() == "events":
            self.data_format = DataFormat.EVENTS
            ObserveInc.events_api_key = observeinc_config['customer_id']
            ObserveInc.set_api_endpoint(observeinc_config['ingest_token'], observeinc_config['path'])
        else:
    ...
    def process_logs(logs, labels):
        ...
        for log_file_obj in logs:
            ...
            status_code = ObserveInc.post_logs(nr_session, payload)
            if status_code != 202:
                print(f'ObserveInc logs api returned code- {status_code}')
            else:
    ...
    def process_events(logs, labels):
        ...
        for log_file_obj in logs:
            ...
            # since the max number of events that can be posted in a single payload to ObserveInc is 2000
            max_events = 2000
            x = [log_events[i:i + max_events] for i in range(0, len(log_events), max_events)]

            for log_entries_slice in x:
                status_code = ObserveInc.post_events(nr_session, log_entries_slice)
                if status_code != 200:
                    print(f'ObserveInc events api returned code- {status_code}')
                else:
                    log_type = log_file_obj['log_type']
                    log_file_id = log_file_obj['Id']
